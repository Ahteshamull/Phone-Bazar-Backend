...
export const updatePhone = asyncHandler(async (req, res) => {
    ...
    if (patch.imei) {
        ...
        if (exists)
            throw new AppError(`Phone with IMEI ${patch.imei} already exists`, 409, "DUPLICATE_IMEI");
    }
    const current = await PhoneModel.findById(id);
    if (!current)
        throw new AppError("Phone stock record was not found", 404, "PHONE_NOT_FOUND");
